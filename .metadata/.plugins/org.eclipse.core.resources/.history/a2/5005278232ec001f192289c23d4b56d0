...
//   DI
	...
	public String memberWriteForm(
			@RequestParam(value = "num", required = false) Integer num, // CRUD U를 하기 위해서 Model쪽에서 num이라는 애를 1 증가한게 아닌 등록된 num
			// 그 자체를 컨트롤러로 몰래 안보이게 넘겨줘야 시퀀스가 1 증가한게 아닌게 되어서 신규 등록이 아닌 수정이 되어진다. 그걸 위해서 RequestParam을 사용한것. 
			Model model ) {
...
//	회원 리스트 7 ->회원 존재 여부 - existsByName()
	@GetMapping(value="/jpa/memberList")
	public String memberList7( Model model, Pageable pageable, 
...
//			members = memberRepository.findByName(searchKeyword, pageable);
	...
	public String memberDelete( @RequestParam(value = "num", required = false) Integer num ) {
		// 삭제처리시 DB에 있는 애를 가져와서 삭제 하는것이고, memberList.html에 삭제 버튼을 눌러서 삭제가 되는거라, form하곤 관련이
		//없어 보여서 @RequestParam 을 사용하는 것이 이해가 안간다. 하지만, 그냥 그렇게 외우면 될것같다. 뭐 이해 안가는게 당연하긴 하다.
